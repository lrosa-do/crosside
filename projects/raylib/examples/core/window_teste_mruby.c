...
execute_file(mrb_state *mrb, const char* input)
{
 ...
 // c->dump_result = TRUE;
  ...
  mrbc_filename(mrb, c, input);
   ...
   //result = 
//mrb_load_irep_file_cxt(mrb, infile, c);
...
int main()
{
  ...
  v = mrb_load_string(mrb, "puts 'hello world'");
  if (mrb->exc)
  {
   mrb_print_error(mrb);
  } else
 ...
 v = mrb_load_string(mrb, " for i in 1..10 do puts ' d ' end ");
  if (mrb->exc)
  {
   mrb_print_error(mrb);
printf("* Error  \n");
...
puts ' end'
    ...
    mrb_value v = mrb_load_string_cxt(mrb,scriptData, NULL);
*/
/*
  if (mrb->exc)
  {
...
void UpdateDrawFrame(void)
{
    ...
    BeginDrawing();
        ...
        DrawText("Congrats! ruby in web!", 190, 200, 20, LIGHTGRAY);
